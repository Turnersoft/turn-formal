...
// use crate::turn_render::Identifier; // already imported above; avoid duplicate
...
pub enum Group {
    ...
    Interception(InterceptionGroup),

    // --- Subgroups as Concrete Objects ---
    SubGroup(SubGroup),
}
...
impl Instantiable for GroupRelation {
    fn instantiate(
        &self,
        _target_context: &Vec<ContextEntry>,
        pattern: &Self,
        _pattern_context: &Vec<ContextEntry>,
    ) -> std::collections::HashMap<Identifier, InstantiationType> {
        // Minimal: treat GroupRelation as atomic for instantiation now.
        // If variants match, no additional bindings; otherwise, empty.
        let mut instantiations = std::collections::HashMap::new();
        if std::mem::discriminant(self) != std::mem::discriminant(pattern) {
...
pub struct QuotientGroup {
    ...
    pub core: GenericGroup,
    /// The group
    pub group: Located<Group>,
    /// The normal subgroup
    pub normal_subgroup: Located<Group>,
...
pub struct KernelGroup {
    pub core: GenericGroup,
    pub defining_homomorphism: Located<GroupHomomorphism>,
...
pub struct CenterGroup {
    pub core: GenericGroup,
    pub parent_group: Located<Group>,
}
...
pub struct NormalizerGroup {
    pub core: GenericGroup,
    pub parent_group: Located<Group>,
    pub subgroup_normalized: Located<Group>,
}
...
pub struct WreathProductGroup {
    pub core: GenericGroup,
    pub base_group: Located<Group>,
    pub acting_group: Located<Group>,
...
pub struct CentralProductGroup {
    pub core: GenericGroup,
    pub component_groups: Vec<Located<Group>>,
...
pub struct PullbackGroup {
    pub core: GenericGroup,
    pub source_groups: Vec<Located<Group>>, // Groups being mapped from
    pub target_group: Located<Group>,       // Group being mapped to
...
/// A group constructed as the intersection of two subgroups
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct InterceptionGroup {
    pub core: GenericGroup,
    pub parent_group: Located<Group>,
    pub first_subgroup: Located<Group>,
    pub second_subgroup: Located<Group>,
    pub interception_props: VariantSet<InterceptionProperty>,
}

/// Properties specific to interception groups
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub enum InterceptionProperty {
    /// The intersection is non-trivial
    NonTrivial,
    /// The intersection is trivial
    Trivial,
    /// The intersection has a specific order
    Order(usize),
    /// The intersection is normal in the parent group
    Normal,
    /// The intersection is characteristic in the parent group
    Characteristic,
}

/// A subgroup of another group
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub struct SubGroup {
    /// The parent group that this is a subgroup of
    pub parent_group: Located<Group>,

    /// The core algebraic group structure
    pub core: GenericGroup,
    /// Properties specific to the subgroup relationship
    pub subgroup_props: VariantSet<SubGroupProperty>,
}

/// Properties specific to subgroups
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub enum SubGroupProperty {
    /// Normal subgroup property
    Normal(NormalSubGroupVariant),
    /// Characteristic subgroup property
    Characteristic(CharacteristicSubGroupVariant),
    /// Index property
    Index(usize),
    /// Order property
    Order(usize),
...
/// Property variants for normal subgroups
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub enum NormalSubGroupVariant {
...
/// Property variants for characteristic subgroups
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Hash)]
pub enum CharacteristicSubGroupVariant {
    /// Is a characteristic subgroup
    Characteristic,
    /// Is not a characteristic subgroup
    NotCharacteristic,
}
...
impl Group {
    pub fn get_variant_name(&self) -> &'static str {
        match self {
            ...
            Group::Interception(_) => "Interception",
...
// SameRole for Group encodes refinement: self can replace baseline iff same variant
// and only collection-like fields extend; all other fields are identical.
impl SameRole for Group {
    fn same_role(
        &self,
        _target_context: &Vec<crate::subjects::math::formalism::proof::ContextEntry>,
        baseline: &Self,
        _baseline_context: &Vec<crate::subjects::math::formalism::proof::ContextEntry>,
    ) -> bool {
        use Group::*;
        match (self, baseline) {
            (Generic(a), Generic(b)) => {
                // base_set, operation must be identical; props can extend (superset)
                a.base_set == b.base_set
                    && a.operation == b.operation
                    && b.props.is_subset(&a.props)
            }
            (SubGroup(a), SubGroup(b)) => {
                // parent_group must be identical; subgroup_props can extend (superset)
                a.parent_group == b.parent_group && b.subgroup_props.is_subset(&a.subgroup_props)
            }
            (Product(a), Product(b)) => {
                // For brevity: same operation, same components, allow product_props to extend
                a.operation == b.operation
                    && a.components == b.components
                    && a.normal_component == b.normal_component
                    && b.product_props.is_subset(&a.product_props)
            }
            (Quotient(a), Quotient(b)) => {
                a.group == b.group
                    && a.normal_subgroup == b.normal_subgroup
                    && b.quotient_props.is_subset(&a.quotient_props)
            }
            (Topological(a), Topological(b)) => {
                a.core == b.core && a.topology == b.topology && b.props.is_subset(&a.props)
            }
            (Lie(a), Lie(b)) => {
                a.core == b.core && a.topology == b.topology && b.props.is_subset(&a.props)
            }
            (GeneralLinear(a), GeneralLinear(b)) => {
                a.core == b.core
                    && a.dimension == b.dimension
                    && a.field == b.field
                    && b.matrix_props.is_subset(&a.matrix_props)
                    && b.linear_props.is_subset(&a.linear_props)
            }
            (SpecialLinear(a), SpecialLinear(b)) => {
                a.general_linear == b.general_linear
                    && b.special_linear_props.is_subset(&a.special_linear_props)
            }
            ...
            (SpecialOrthogonal(a), SpecialOrthogonal(b)) => {
                a.orthogonal == b.orthogonal
                    && b.special_orthogonal_props
                        .is_subset(&a.special_orthogonal_props)
            }
            (Unitary(a), Unitary(b)) => {
                a.core == b.core
                    && a.dimension == b.dimension
            ...
            (SpecialUnitary(a), SpecialUnitary(b)) => {
                a.unitary == b.unitary
                    && b.special_unitary_props.is_subset(&a.special_unitary_props)
            }
            (ModularAdditive(a), ModularAdditive(b)) => {
                a.core == b.core
                    && a.modulus == b.modulus
                    && b.modular_props.is_subset(&a.modular_props)
            }
            (ModularMultiplicative(a), ModularMultiplicative(b)) => {
                a.core == b.core
                    && a.modulus == b.modulus
                    && b.modular_props.is_subset(&a.modular_props)
            }
            (Symmetric(a), Symmetric(b)) => a.core == b.core && a.degree == b.degree,
            (Alternating(a), Alternating(b)) => a.core == b.core && a.degree == b.degree,
            (Dihedral(a), Dihedral(b)) => a.core == b.core && a.order == b.order,
            (Cyclic(a), Cyclic(b)) => {
                a.core == b.core && a.generator == b.generator && a.order == b.order
            }
            (Free(a), Free(b)) => {
                a.core == b.core && a.rank == b.rank && b.free_props.is_subset(&a.free_props)
            }
            (Center(a), Center(b)) => a.parent_group == b.parent_group && a.core == b.core,
            (GeneratedSubgroup(a), GeneratedSubgroup(b)) => {
                a.parent_group == b.parent_group && a.generators == b.generators && a.core == b.core
            }
            (Normalizer(a), Normalizer(b)) => {
                a.parent_group == b.parent_group
                    && a.subgroup_normalized == b.subgroup_normalized
                    && a.core == b.core
            }
            (Centralizer(a), Centralizer(b)) => {
                a.parent_group == b.parent_group
                    && a.element_centralized == b.element_centralized
                    && a.core == b.core
            }
            (CommutatorSubgroup(a), CommutatorSubgroup(b)) => {
                a.parent_group == b.parent_group && a.core == b.core
            }
            (SylowSubgroup(a), SylowSubgroup(b)) => {
                a.parent_group == b.parent_group && a.prime == b.prime && a.core == b.core
            }
            (WreathProduct(a), WreathProduct(b)) => {
                a.base_group == b.base_group && a.acting_group == b.acting_group && a.core == b.core
            }
            (CentralProduct(a), CentralProduct(b)) => {
                a.component_groups == b.component_groups && a.core == b.core
            }
            (Pullback(a), Pullback(b)) => {
                a.source_groups == b.source_groups
                    && a.target_group == b.target_group
                    && a.defining_homomorphisms == b.defining_homomorphisms
                    && a.core == b.core
            }
            (Restriction(a), Restriction(b)) => {
                a.parent_group == b.parent_group
                    && a.restriction_description == b.restriction_description
                    && a.core == b.core
            }
            (Interception(a), Interception(b)) => {
                a.parent_group == b.parent_group
                    && a.first_subgroup == b.first_subgroup
                    && a.second_subgroup == b.second_subgroup
                    && a.core == b.core
                    && b.interception_props.is_subset(&a.interception_props)
            }
            _ => false,
        }
    }
